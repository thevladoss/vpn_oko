...
fix:
  ...
  fixed: 7
  skipped: 3
  fixed_ids: [H-01, M-01, M-02, M-03, L-01, L-02, L-06]
  skipped_ids: [L-03, L-04, L-05]
  analyze: clean
  tests: 46 passed
---
...
**Статус:** ИСПРАВЛЕНО (f1895cc). Обе ветки ловят `PlatformException` → `mapPlatformException`; тесты покрывают `disconnect`/`syncStatus`.
...
**Статус:** ИСПРАВЛЕНО (7c8ad8d). Подписка на broadcast-контроллер оформляется в `onListen` синхронно, до отдачи снапшота/буфера; без новых пакетов. Регресс-тесты гонки для статуса и логов.
...
**Статус:** ИСПРАВЛЕНО частично (95cda7c). Маппер `errorToEntity(ErrorMessage)` теперь пробрасывает реальный `message` в `VpnError`. Подписка `errorEvents` в поток статусов репозитория отложена до Phase 3 (появление консюмера) — вне echo-скоупа фазы.
...
**Статус:** ИСПРАВЛЕНО (7b3e58a). `dispose()` объявлен в контрактах `VpnRepository`/`LogRepository`; `AppDependencies.dispose()` закрывает репозитории до `_bridge.dispose()`. Тесты на идемпотентное закрытие.
...
**Статус:** ИСПРАВЛЕНО (b31a5e5). `PlatformFailure` удалён из иерархии `Failure`.
...
**Статус:** ИСПРАВЛЕНО (9259ab6). `VpnEventBus.emit` обновляет `snapshot` и на `TrafficChangedMessage`, симметрично Swift. Native-код, Dart-тестами не покрывается (нет Kotlin-харнесса в echo-фазе); проверено паритетом со Swift.
...
**Статус:** ПРОПУЩЕНО. Вне запрошенного скоупа фикса; текущее поведение терпимо (без `ArgumentError`). Нормализация алиасов отложена.
...
**Статус:** ПРОПУЩЕНО. Касается debug-harness в `main.dart`, вне echo-скоупа фазы. В echo native не бросает; актуально при переходе на реальный слой.
...
**Статус:** ПРОПУЩЕНО. Не запрошено; ввод отдельных типов ошибок (`VpnStopFailure`) отложен. После H-01 ошибки `disconnect`/`syncStatus` приезжают как `VpnStartFailure` — приемлемо для echo-фазы, помечено на будущее.
...
**Статус:** ИСПРАВЛЕНО (b094cd3). `VpnConnected.connectedSince` стал nullable; маппер отдаёт `null` вместо эпохи 0. Тест обновлён.
