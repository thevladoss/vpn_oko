...
- [x] **BRG-03**: Domain-слой изолирован от кодогена: типы Pigeon получают суффикс Message, импорт *.g.dart разрешён только в core/bridge и features/*/data, мапперы переводят DTO в доменные entity
...
- [x] **AND-05**: Read-loop TUN-дескриптора считает реальные байты и шлёт trafficChanged раз в секунду
- [x] **AND-06**: Все переходы состояний и ключевые действия сервиса логируются событиями logMessage во Flutter

### iOS

- [x] **IOS-01**: Swift-реализация Pigeon-моста в основном приложении: startVpn/stopVpn/getStatus и поток событий работают, демо на iOS показывает статусы/логи из Swift-слоя
...
- [x] **IOS-03**: Статусы туннеля из extension доводятся до приложения (NEVPNStatus observer) и дальше во Flutter
